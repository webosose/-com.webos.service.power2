                        ...
                        GSourceFunc callback, gpointer user_data)
{
        ...
        if (!callback)
        {
                g_critical("Timeout source dispatched without callback, Call g_source_set_callback()");
                return FALSE;
        }
                            ...
                            gboolean from_poll)
{
        ...
        if (!from_poll)
        {
                ...
                if (!context)
                {
                        g_critical("Cannot get context for timer_source. Maybe you didn't call g_source_attach()");
                        return;
                }
